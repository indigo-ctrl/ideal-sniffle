...
API_KEY = os.getenv("API_KEY")
 # ВСТАВЬТЕ ТОКЕН БОТА
client = Mistral(api_key="ZhyMUqlfL9jFt1WOdrGVIMamL0nO6pqk") # 3AMEHиTE AПи Kлч HA BAW https://console.mistral.ai/api-keys
...
async def filter_messages (message: Message):
    ...
    chat_response = client.chat.complete(
        ...
        messages=[
            {
                "role": "system",
                "content": f"Crенерируй ответ о погоде в городе {ai_response.city} на основании предоставленны дaнныx: {str(weather_info)}",
            }
        ]
        )
